...
public class Torus {
	...
	public void push( State newState , int depLimit , Stack stack , ArrayList<State> prefix )
	{
					 	if(newState.depth < depLimit) { 
				 		State[] tmp = newState.getSuccessors();
				 		Arrays.sort( newState );
				 		// path-checking cycle prevention.
				 		for (int i = 0; i < tmp.length; i++){  
				 			boolean unique = true;
				 			for (int j = 0; j < prefix.size(); j++) {
				 				if( prefix.get(j).equals(tmp[i]) ){
									unique = false;
									break;
								}
								//loops++;
